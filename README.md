...
A browser-based neon target-shooting game built around a canvas-based aiming and scoring loop.
...
- Cursor-based aiming.
- animated targets and impact effects.
- score, accuracy, combo, and streak tracking.
- recording-oriented presentation mode.

## Project Structure

The primary entry point is `index.html`. Supporting components, services, styles, and configuration files remain organized next to the relevant application layer.
...
```text
Open `index.html` in a browser or serve the directory with a static web server.
```
